import cv2 # type: ignore #open cv
import imutils # type: ignore
from imutils import face_utils # type: ignore #it is used for landmarks of left and right eye
...
flag = 0 #frame count
frame_check = 6 #if eye blinks that much(means eyes closing) then alarm will start beeping
     ...
     for subject in subjects:
          ...
          if ear<thresh : #thresh is a certain value from which if the ear value is less then it is considered as close / blink eye
               ...
               if flag>=frame_check: #if ear will be less that continuously for frame_check count, this will help to beep alarm when closes not just blinks
                    #warning message
                    cv2.putText(frame, "Careful you are sleepy", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    print("Finding Sleepy")
                    #(10,30) coordinates of x and y axis
                    #FONT_HERSHEY_SIMPLEX - font
                    #0.7 - thickness of font
                    # (0, 0, 255) - color of font
                    # 2 - thickness of line
                    cv2.putText(frame, "Careful you are sleepy", (10,325), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    print("WAKE UP")
